...
from main import WebMotionAnalyzer
...
# Add a global analyzer per camera
CAMERA_ANALYZERS = {}
...
def initialize_camera(camera_id, camera_index):
    ...
    try:
        ...
        analyzer = WebMotionAnalyzer()
        analyzer.set_zone_points(camera.zone_points if camera and camera.zone_points else DEFAULT_ZONE_POINTS)
        
        # Store camera objects
        CAMERAS[camera_id] = cap
        CAMERA_DETECTORS[camera_id] = analyzer
        CAMERA_PREVIOUS_FRAMES[camera_id] = None
        CAMERA_PREVIOUS_PERSON_COUNTS[camera_id] = 0
        CAMERA_LAST_DETECTION_TIMES[camera_id] = 0
        CAMERA_ANALYZERS[camera_id] = analyzer
...
def process_frame(frame, camera_id):
    ...
    if not analyzer:
        return frame, False
    # Always use the analyzer's zone_points
    roi_points = analyzer.zone_points or DEFAULT_ZONE_POINTS
    result = analyzer.analyze_frame(frame, roi_points)
    processed_frame = result['frame']
    detected = result['detected']
    return processed_frame, detected
